/**
 * Metastore API Route Handler
 *
 * Proxies requests to the metastore-service backend.
...
const METASTORE_SERVICE_URL = process.env.METASTORE_SERVICE_URL ?? "http://localhost:8002";

/**
 * Handle all HTTP methods for metastore (feature flags, metadata, config)
 */
...
/**
 * Core request handler that proxies to metastore-service
 */
async function handleRequest(
  ...
  try {
    ...
    const backendUrl = new URL(`/api/v1/${path}`, METASTORE_SERVICE_URL);
    ...
    console.error("Metastore API proxy error:", error);
      ...
      return NextResponse.json(
        {
          ...
          message: "Unable to connect to metastore service",
          statusCode: 503,
        },
        { status: 503 }
      );
