...
class TestInMemoryNotificationChannel:
    ...
    async def test_send_batch(self):
        ch = InMemoryNotificationChannel()
        items = [
            Notification(channel="test", recipient=f"u{i}@x.com")
            for i in range(5)
        ]
...
class TestNotificationService:
    async def test_send_via_registered_channel(self):
        ...
        result = await svc.send(
            Notification(channel="email", recipient="u@x.com", subject="Hi")
        )
        assert result.status == NotificationStatus.SENT
        assert len(ch.sent) == 1
    ...
    async def test_send_unknown_channel(self):
        svc = NotificationService()
        result = await svc.send(
            Notification(channel="sms", recipient="+123")
        )
        assert result.status == NotificationStatus.FAILED
    ...
    async def test_send_batch(self):
        ...
        items = [
            Notification(channel="email", recipient=f"u{i}@x.com")
            for i in range(3)
        ]
